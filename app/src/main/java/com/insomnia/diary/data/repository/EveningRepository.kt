...
class EveningRepository(private val db: InsomniaDatabase) {
    ...
    suspend fun saveCustomType(label: String) =
        dao.insertCustomType(CustomEventTypeEntity(label = label))
    ...
    suspend fun saveCustomMood(label: String) =
        dao.insertCustomMood(CustomMoodEntity(label = label))
    ...
    suspend fun findById(id: Long): EveningProtocol? =
    ...
    suspend fun update(id: Long, protocol: EveningProtocol) = db.withTransaction {
        dao.deleteById(id)
        insertAll(protocol)
    }
...
private fun DayEventWithAttendees.toDomain() = DayEvent(
    ...
    batteryLevel = Percentage(event.batteryLevel ?: 70),
